...
#import LFMM results
lfmm_results = read.table("data/LFMM/ctmax.lfmm.txt", header = T)
...
lfmm_results.sig$distance %>% abs %>% mean #3201.167
lfmm_results.sig$distance %>% abs %>% median #2027
lfmm_results.sig %>% filter(abs(distance) > 0) %>% pull(distance) %>% abs %>% range #109 9178
lfmm_results.sig %>% filter(match.type == "in.gene") %>% nrow #1
lfmm_results.sig %>% nrow #1 of 6 in gene

gff.dt %>% filter(sig_SNP_count > 0) #6 genes have SNPs
...
p1
#pdf("figures/SNP_distance.topt.pdf")
#p1
#dev.off()
...
p2
#pdf("figures/SNP_per_gene.topt.pdf")
#p2
#dev.off()
...
##############################
write.table(gff.dt %>% filter(sig_SNP_count > 0), "data/LFMM/annotations/ctmax.gene_SNP_hits.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(lfmm_results.sig, "data/LFMM/annotations/ctmax.SNPs.gene_found.nearest_neighbors.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(gff.dt %>% filter(sig_SNP_count > 0) %>% select(geneID), "data/LFMM/annotations/ctmax.geneIDs.nearest_neighbors.txt", col.names = F, row.names = F, sep = "\t", quote = F)
