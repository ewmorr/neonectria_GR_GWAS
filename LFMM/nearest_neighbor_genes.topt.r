...
#import LFMM results
lfmm_results = read.table("data/LFMM/topt.lfmm.txt", header = T)
...
##############################
write.table(gff.dt %>% filter(sig_SNP_count > 0), "data/LFMM/annotations/topt.gene_SNP_hits.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(lfmm_results.sig, "data/LFMM/annotations/topt.SNPs.gene_found.nearest_neighbors.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(gff.dt %>% filter(sig_SNP_count > 0) %>% select(geneID), "data/LFMM/annotations/topt.geneIDs.nearest_neighbors.txt", col.names = F, row.names = F, sep = "\t", quote = F)
