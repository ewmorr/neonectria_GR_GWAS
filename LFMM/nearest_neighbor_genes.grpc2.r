...
#import LFMM results
lfmm_results = read.table("data/LFMM/grpc2.lfmm.txt", header = T)
...
lfmm_results.sig$distance %>% abs %>% mean #3822.029
lfmm_results.sig$distance %>% abs %>% median #4143
lfmm_results.sig %>% filter(abs(distance) > 0) %>% pull(distance) %>% abs %>% range #80 11153
lfmm_results.sig %>% filter(match.type == "in.gene") %>% nrow #4
lfmm_results.sig %>% nrow #4 of 34 in gene

gff.dt %>% filter(sig_SNP_count > 0) #22 genes have SNPs
gff.dt %>% filter(sig_SNP_count > 1) #6 have at least 2 SNPs
nrow(gff.dt)
